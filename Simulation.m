    ...
    methods
        ...
        function error=plan(obj,SOLVER,goal)
            ...
            for i=1:length(obj.list_of_operating_robots)
                ...
                numbeer_of_charging_windows=floor(obj.list_of_operating_robots(i).simulation_time/obj.list_of_operating_robots(i).battery_life);
                temp=[];
                for j=1:numbeer_of_charging_windows
                    iter=iter+1;
                    temp=[temp iter];
                    
                    tempSet=cat(1,tempSet,ones(length(obj.list_of_operating_robots(i).charging)/numbeer_of_charging_windows,1)*iter);
                end
                OperatingRobotsets(i,:)=temp;
            end
